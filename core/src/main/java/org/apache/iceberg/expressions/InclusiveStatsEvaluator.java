...
public class InclusiveStatsEvaluator {
  ...
  public InclusiveStatsEvaluator(Schema schema, Expression unbound, boolean caseSensitive) {
    ...
    this.expr = Binder.bind(schema.asStruct(), rewritten, caseSensitive);

    Expression transformed = ExpressionUtil.toSignedUUIDLiteral(rewritten);
    this.signedUuidExpr =
        transformed != null ? Binder.bind(schema.asStruct(), transformed, caseSensitive) : null;
  }
  ...
  public boolean eval(ContentStats stats, long recordCount) {
    boolean result = new StatsEvalVisitor().eval(stats, recordCount, expr, false);
    if (result || signedUuidExpr == null) {
      return result;
    }

    return new StatsEvalVisitor().eval(stats, recordCount, signedUuidExpr, true);
  }
